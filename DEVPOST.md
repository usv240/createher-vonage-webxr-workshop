...
## Testing
The layout is verified in a real browser at 390 / 1280 / 1920 / 3840 px in both overlay states —
no overlaps, nothing `position: fixed`, no viewport overflow — and the narration's word-sync maths
and the story data are unit tested. Both suites run without Vonage credentials.
...
## What's next
Pilot with United Through Reading and prison family-literacy programs; per-family rooms keyed by dialed number; licensed picture books; caregiver approval of new callers via Vonage Verify; illustrator-drawn scenes.
